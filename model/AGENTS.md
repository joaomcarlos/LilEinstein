...
- The explicit queue remains authoritative for normal selection; only a materially PACK-BOUND active technology may widen the temporary-candidate search to scored eligible technologies outside that queue, while preserving the original queued target for recovery
...
- Active science-supply checks use staggered live lab-starvation snapshots and the same material-loss threshold as the player-facing PACK-BOUND diagnostic; non-active candidate and recovery checks project stock plus production against that technology's physical compatible-lab pack demand over the bounded forecast horizon
