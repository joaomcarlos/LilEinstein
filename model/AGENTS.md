...
- Active science-supply checks use staggered live lab-starvation snapshots and the same material-loss threshold as the player-facing PACK-BOUND diagnostic; inactive checks share one full-capacity estimate for physical demand and completion horizon, and cluster-mode recovery requires enough stock in the sole compatible consuming scope rather than counting incompatible remote stock, force-wide production, or a one-time delivery burst
