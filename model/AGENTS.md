...
- `research_weights.lua` — editable AI scoring tables for tech prioritization; designed to be regenerated via external AI chat (see file header). Contains `research_weights` (per-tech importance), `research_caps` (hard level caps including practical infinite-tech limits from abucnasty breakpoint analysis), `pack_difficulty` (per-science-pack UPS/logistics cost weights used in `queue.score_tech_detailed`), and `espm_constants`/`espm_multiplier` (reference ESPM formula constants for display/target calculations; runtime always uses live API values)
...
- Temp-tech switching respects pinned techs, science priority, and a symmetric score margin (1.15×); unavailable techs are scored with a discount and never act as runtime candidates
- The explicit queue remains authoritative for normal selection; only a materially PACK-BOUND active technology may widen the temporary-candidate search to a fixed-size scored slice of eligible technologies outside that queue, while preserving the original queued target for recovery. The target is the tech we want to return to once science is available: when a regular queue tech goes pack-bound, it becomes the new target; when the temp tech itself goes pack-bound (both target and temp starving), the original target is preserved so the system still tries to return to it after the replacement temp finishes
- When no fully-sufficient candidate exists (every candidate uses at least one bottleneck science), a partial-supply fallback selects the queued candidate with the fewest bottleneck sciences, requiring strictly fewer than the current technology; non-bottlenecked sciences must still be physically available. This prevents idle lab capacity when a candidate would run at higher (but not full) coverage
...
- `queue.score_tech_detailed` weights total cost by `rw.pack_difficulty` (per-science-pack UPS/logistics cost) so an 8-pack promethium tech scores as genuinely more expensive than a 4-pack Nauvis tech, beyond just ingredient count. `rw.research_caps` includes practical infinite-tech limits derived from abucnasty breakpoint analysis (one-shot combat thresholds, exponential cost walls, game-capped railgun shooting speed at 12); `rw.espm_constants`/`rw.espm_multiplier` are reference-only — runtime productivity/drain always reads live API values
- Lab capacity (`get_lab_capacity_spm`) uses `lab_entity.productivity_bonus` which already includes force-based bonuses (`force.laboratory_productivity_bonus`) and beacon/module bonuses per the Factorio API; it must not multiply by `force.laboratory_productivity_bonus` separately or it double-counts the force productivity bonus
- Active science-supply checks use staggered live lab-starvation snapshots and the same material-loss threshold as the player-facing PACK-BOUND diagnostic; the bottleneck only includes sciences whose individual per-pack lost SPM exceeds the material threshold, so a trivially-starved science (e.g. 1 of 500 labs missing automation) does not block switching to candidates that also use it. Inactive checks share one full-capacity estimate for physical demand and completion horizon, scaled per-science by the working fraction when a bottleneck has been measured so candidates using starved sciences are not rejected for overestimated demand. Cluster-mode recovery requires enough stock in the sole compatible consuming scope rather than counting incompatible remote stock, force-wide production, or a one-time delivery burst
...
- The per-force research diagnostic measures only samples for the current technology, classifies decision states from named material-loss thresholds, and emits deterministic semantic display clusters without persisting logistic networks. Utilization uses `min(actual_spm, working_spm) / expected_spm` so it reflects the true working fraction even when actual throughput exceeds expected (fast labs) while many labs are idle; `measured_utilization` retains the raw `actual_spm / expected_spm` clamp for comparison
...
- Upcoming research plans put the active technology first and prefer science-supplied candidates before science-blocked fallbacks. Wait times accumulate only for available (non-blocked) technologies, so a blocked tech does not inflate the wait time of techs behind it in the plan
