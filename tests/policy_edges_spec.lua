...
local tests = {
    {"handles absent stores, recursive copies, and all setting sanitizers", function()
        ...
        t.assert_equal(policy.set_setting(1, "min_switch_seconds", math.huge), true)
        t.assert_equal(policy.get_setting(1, "min_switch_seconds"), 5)
        t.assert_equal(policy.set_setting(1, "plan_horizon_minutes", math.huge), true)
        t.assert_equal(policy.get_setting(1, "plan_horizon_minutes"), 1)
    ...
    ,{"covers approved control-center bound sanitizers and plan_horizon_minutes", function()
        reset()
        local sanitize_setting = find_private(policy.set_setting, "sanitize_setting")
        t.assert_true(type(sanitize_setting) == "function")
        local min_sw_lo, min_sw_valid = sanitize_setting("min_switch_seconds", 2)
        t.assert_true(min_sw_valid)
        t.assert_equal(min_sw_lo, 5)
        local min_sw_hi, _ = sanitize_setting("min_switch_seconds", 600)
        t.assert_equal(min_sw_hi, 30)
        local min_sw_nil, _ = sanitize_setting("min_switch_seconds", nil)
        t.assert_equal(min_sw_nil, 5)
        local fc_hi, fc_valid = sanitize_setting("forecast_seconds", 3600)
        t.assert_true(fc_valid)
        t.assert_equal(fc_hi, 300)
        local fc_lo, _ = sanitize_setting("forecast_seconds", -5)
        t.assert_equal(fc_lo, 0)
        local fc_nil, _ = sanitize_setting("forecast_seconds", nil)
        t.assert_equal(fc_nil, 0)
        local ph_default = sanitize_setting("plan_horizon_minutes", 30)
        t.assert_equal(ph_default, 30)
        local ph_lo, ph_valid = sanitize_setting("plan_horizon_minutes", 0)
        t.assert_true(ph_valid)
        t.assert_equal(ph_lo, 1)
        local ph_hi, _ = sanitize_setting("plan_horizon_minutes", 99999)
        t.assert_equal(ph_hi, 300)
        local ph_nil, _ = sanitize_setting("plan_horizon_minutes", nil)
        t.assert_equal(ph_nil, 1)
        local ph_bad, _ = sanitize_setting("plan_horizon_minutes", "bad")
        t.assert_equal(ph_bad, 1)
        local ph_frac, _ = sanitize_setting("plan_horizon_minutes", 45.9)
        t.assert_equal(ph_frac, 45)
        t.assert_equal(policy.get_setting(1, "plan_horizon_minutes"), 30)
        t.assert_true(policy.set_setting(1, "plan_horizon_minutes", 120))
        t.assert_equal(policy.get_setting(1, "plan_horizon_minutes"), 120)
        local exported = policy.export_settings(1)
        t.assert_equal(exported.settings.plan_horizon_minutes, 120)
        local sanitized = policy.sanitize_settings({settings = {plan_horizon_minutes = 500}})
        t.assert_equal(sanitized.settings.plan_horizon_minutes, 300)
        t.assert_true(policy.import_settings(1, {settings = {plan_horizon_minutes = 15}}))
        t.assert_equal(policy.get_setting(1, "plan_horizon_minutes"), 15)
    end}
}
